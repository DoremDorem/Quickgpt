...
const Sidebar = ({isMenuOpen,setIsMenuOpen}) => {
    ...
    const {chats,setSelectedChat,theme,setTheme,user,navigate,
      createNewChat,axios,setChats,fetchUserChats,setToken,token
    }=useAppContext();
  ...
  return (
    ...
    'max-md:-translate-x-full'}`}>
      ...
      <button onClick={createNewChat} className='flex justify-center items-center w-full py-2 mt-10
       text-white bg-gradient-to-r from-[#A456F7] to-[#3D81F7] text-sm rounded-md
       cursor-pointer'>
      ...
      dark:border-white/20 rounded-md'>
       ...
       <input onChange={(e)=>setSearch(e.target.value)} value={search} type="text" placeholder='search convertation' className='text-sx
